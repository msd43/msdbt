...
try {
    ...
    console.log("🌍 MSD Veri Tabanına Bağlı 🐉")
} catch {
...
`)
                ...
                for (let jid of mentionUser) {
                    ...
                    reply(`
Merhaba ${citel.pushName} \n\n, bu *${tlang().title}* bir bot.
Onu etiketlemeyin, şu anda meşgul. Ama merak etmeyin sizi temin ederim en kısa sürede ona haber vereceğim.😉.
...
`.trim());
                ...
                if (isCreator && citel.text.startsWith('$')) {
                    let code = budy.slice(2)
                    if (!code) {
                        citel.reply(`Master'ı çalıştırmam için bana bir sorgu sağlayın!`);
                        return;
                    }
                    try {
...
app.listen(port, () => console.log(`MSD Server bağlantı noktasını dinliyor http://localhost:${port}!`));
